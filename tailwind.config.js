...
module.exports = {
  ...
  theme: {
    extend: {
      ...
      boxShadow: {
  'card': '0 4px 6px rgba(0, 0, 0, 0.1)', // Apply only for card-specific shadows
  '3xl': '14px 17px 40px 4px rgba(0, 0, 0, 0.2)',
  'inset': 'inset 0px 18px 22px rgba(0, 0, 0, 0.1)',
  'darkinset': '0px 4px 4px inset rgba(0, 0, 0, 0.2)',
},
      backgroundImage: {
  ...
  plugins: [
    plugin(function ({ addBase, theme }) {
      function addVariablesForColors(colorObj, colorGroup = '') {
        return Object.keys(colorObj).reduce((vars, key) => {
          const value = colorObj[key];
          const cssVarName = `--color${colorGroup ? `-${colorGroup}` : ''}-${
            key === 'DEFAULT' ? '' : `-${key}`
          }`;
          if (typeof value === 'string') {
            vars[cssVarName] = value;
          } else {
            Object.assign(vars, addVariablesForColors(value, key));
          }
          return vars;
        }, {});
      }
      ...
      addBase({
        ':root': addVariablesForColors(theme('colors')),
      });
    }),
  ],
};
